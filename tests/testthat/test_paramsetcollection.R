...
p1 = ParamSet$new(splitrule = Set$new("logrank","extratrees","C","maxstat") ~ tags("train","predict") + "logrank",
...
test_that("get_values",{
  expect_equal(c$get_values(), list(a_splitrule = "logrank", a_sample.fraction = 0.5,
                                    b_ntrees = 2))
...
test_that("supports",{
  expect_equal(c$supports, list(a_splitrule = Set$new("logrank", "extratrees", "C", "maxstat"),
...
test_that("Values",{
  ...
  expect_silent({c$values$a_splitrule = "C"})
  expect_equal(c$values$a_splitrule, "C")
  expect_error({c$values$a_splitrule = 2}, "does not lie")
  expect_equal(c$.__enclos_env__$private$.sets$a$values$splitrule, "C")
  expect_silent({c$values = list(a_sample.fraction = 0.1)})
